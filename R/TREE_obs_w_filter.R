# TREE_obs_w_filter.R provides a convenience wrapper for retrieving
# FIA tree-level observations using user-specified filter criteria.

#' Retrieve FIA Tree Observations with Filters
#'
#' Returns tree-level observations from FIA databases for a specified
...
#' required SQL filter expression and passing it to \code{TREE_obs()}.
#'
...
#'
#' @return
#' A data frame containing filtered tree-level observations and
#' associated FIA estimate statistics.
#'
...
TREE_obs_w_filter <- function(EVAL_GRP, ATTRIBUTE_NBR, GRP_BY_ATTRIB="CN",
                              SCHEMA = "FS_FIADB",VAR_NAMES = NA, VAR_VALUES = NA, VAR_CONDS = NA, VAR_BOOLS = NA,dbname = "fiadb"){ 
  #Aakriti 01/13/2025
