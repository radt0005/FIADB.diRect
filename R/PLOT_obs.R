# PLOT_obs.R retrieves FIA plot-level observations with optional
# grouping, filtering, and table joins.

#' Retrieve FIA Plot Observations
#'
...
#'
#' This function is intended for examining the plot-level observations
...
#'
#' @return
#' A data frame containing plot-level observations and associated FIA
#' estimate statistics.
#'
#' @examples
#' \dontrun{
...
PLOT_obs <- function(EVAL_GRP, ATTRIBUTE_NBR, GRP_BY_ATTRIB="CN", FILTER = NA, JOINS = NA, SCHEMA = "FS_FIADB",
                     dbname = "fiadb"){                            #Aakriti 09/10/2024
  ...
  #to add CN as default 
  if (all(c("CN", "STATECD") %in% GRP_BY_ATTRIB)){
    GRP_BY_ATTRIB <- GRP_BY_ATTRIB
  } else {
    GRP_BY_ATTRIB <- c(GRP_BY_ATTRIB, "CN", "STATECD")
  }
  ...
  #changing the name "CN" to "PLT_CN"
  names(queryResults)[which(names(queryResults)=="CN")] <-"PLT_CN"
  #getting the state code and their names from survey table (SURVEY)
  #if(STATECD.in.GRP_BY_ATTRIB){
  #Survey_table <- "Select SURVEY.STATECD, SURVEY.STATENM  FROM FS_FIADB.SURVEY"
  #STATE_NAME <- getQuery(Survey_table,con) #Aakriti 09/10/2024
  #queryResults <- dplyr::left_join(queryResults, STATE_NAME)}
