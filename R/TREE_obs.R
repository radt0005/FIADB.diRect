# TREE_obs.R retrieves FIA tree-level observations with optional
# grouping, filtering, and table joins.

#' Retrieve FIA Tree Observations
#'
#' Returns tree-level observations from FIA databases for a specified
#' evaluation group and attribute. Results may be grouped by one or
#' more FIA variables and optionally filtered or joined to additional
#' FIA reference tables.
#'
#' This function is intended for examining the tree-level observations
#' that contribute to FIA estimates and summaries.
#'
...
#'   the observations.
#' @param FILTER Optional SQL filter expression, typically generated by
#'   \code{create_filter()}.
#' @param JOINS Optional join clauses required to access variables not
#'   contained in the default FIA tables.
#' @param SCHEMA PostgreSQL schema containing the FIA tables.
#' @param dbname PostgreSQL database name.
#'
#' @return
#' A data frame containing tree-level observations and associated FIA
#' estimate statistics.
#'
...
TREE_obs <- function(EVAL_GRP, ATTRIBUTE_NBR, GRP_BY_ATTRIB="CN", FILTER = NA,
  ...
  #to add "CN" and "PLT_CN" as default 
  if (all(c("CN", "PLT_CN","STATECD") %in% GRP_BY_ATTRIB)){
  GRP_BY_ATTRIB <- GRP_BY_ATTRIB
  } else {
    GRP_BY_ATTRIB <- c(GRP_BY_ATTRIB, "PLT_CN", "CN", "STATECD")
    }
  ...
  REF_POP_ATTRIBUTE <- getQuery(RPA_query,con) #Aakriti 09/10/2024
  ...
  for(x in 1:length(GRP_BY_ATTRIB)){
    # TODO: verify how to match GRP_BY_ATTRIB to selection table
    # for example: this code matches all of the GRP_BY_ATTRIB attributes to the PLOT table
    # except fortypcd, which is matched to the COND table.
    if(GRP_BY_ATTRIB[x] %in% c("CN", "PLT_CN")){
      GRP_BY_ATTRIB_PT1[x] <- paste0("TREE.",GRP_BY_ATTRIB[x]," ",GRP_BY_ATTRIB[x])
    }else{
      GRP_BY_ATTRIB_PT1[x] <- 
  ...
  GRP_BY_ATTRIB_PT1 = 
    paste(GRP_BY_ATTRIB_PT1,collapse=", ")
  # example for GRP_BY_ATTRIB = c("STATECD","COUNTYCD")
  ### part 2 should be formatted as:
  ### "plot.statecd, plot.countycd"
  # part 2 (GRP_BY_ATTRIB_PT2) has NO aliases
  ...
  for(x in 1:length(GRP_BY_ATTRIB)){
    ...
    # except fortypcd, which is matched to the COND table.
    if(GRP_BY_ATTRIB[x] %in% c("CN", "PLT_CN")){
      GRP_BY_ATTRIB_PT2[x] <- paste0("TREE.",GRP_BY_ATTRIB[x])
    }else{
      GRP_BY_ATTRIB_PT2[x] <- 
        paste0(TABLE_VAR_REF$TABLE[TABLE_VAR_REF$VAR_NAME == GRP_BY_ATTRIB[x]],
               '.',GRP_BY_ATTRIB[x])
  ...
  GRP_BY_ATTRIB_PT2 = 
    paste(GRP_BY_ATTRIB_PT2,collapse=", ")
  # example for GRP_BY_ATTRIB = c("STATECD","COUNTYCD")
  ### part 3 should be formatted as:
  ### "statecd, countycd"
  # part 3 (GRP_BY_ATTRIB_PT2) has ONLY aliases
  ...
  GRP_BY_ATTRIB_PT3 = 
    paste(GRP_BY_ATTRIB_PT3,collapse=", ")
  ...
  queryResults <- getQuery(SQL_QUERY_SE_PART9,con) #Aakriti 09/10/2024
  names(queryResults)[which(names(queryResults)=="GROUP_BY_FIELD")] <- GRP_BY_ATTRIB[length(GRP_BY_ATTRIB)]
  #changing the name "CN" to "TREE_CN"
  names(queryResults)[which(names(queryResults)=="CN")] <-"TREE_CN"
  ...
  if(FORTYPCD.in.GRP_BY_ATTRIB){
    ...
    FOREST_TYPE <- getQuery(RFT,con) #Aakriti 09/10/2024
    queryResults <- dplyr::left_join(queryResults, FOREST_TYPE, dplyr::join_by("FORTYPCD" == "FORTYPCD"))
  }
  ...
  if(COUNTYCD.in.GRP_BY_ATTRIB){
    ...
    COUNTY <- getQuery(RCT,con)  #Aakriti 09/10/2024
    queryResults <- dplyr::left_join(queryResults, COUNTY)
  }
  ...
  if(OWNGRPCD.in.GRP_BY_ATTRIB){
    ...
    OWNERGRP <- getQuery(ROT,con) #Aakriti 09/10/2024
    queryResults <- dplyr::left_join(queryResults, OWNERGRP, dplyr::join_by("OWNGRPCD" == "OWNGRPCD"))
  }
  queryResults <- queryResults |> 
    dplyr::select(-SE_OF_ESTIMATE_PCT, -SE_OF_ESTIMATE, 
           -VAR_OF_ESTIMATE, -TOTAL_PLOTS, 
           -NON_ZERO_PLOTS, -TOT_POP_AC)
